...
st.title("🎰 Generator Variante Loterie 12/66 (configurabil 3/3, 4/4, 5/5...)")
...
with col2:
    ...
    top_count = st.slider("Câte numere din top?", min_value=10, max_value=66, value=50, step=1)
...
strategy = st.radio(
    ...
    [
        ...
        "💥 Triple Fusion (3 din top 34 + 1 din rest)",
        "🌀 Random Pairs (2 perechi aleatoare)",
        "⚡ Frecvență Ponderată (pondere după frecvență)",
        "🎪 Mix Strategy (combinație din toate)"
    ]
)

variant_size_label = st.selectbox(
    "Alege tipul de variantă:",
    ["3/3", "4/4", "5/5", "6/6", "7/7"],
    index=1
)
numere_pe_varianta = int(variant_size_label.split("/")[0])
...
if st.button("🚀 Generează variante"):
    if not st.session_state.top_numbers:
        ...
        while len(variants) < num_variants and attempts < max_attempts:
            ...
            if strategy == "🎯 Standard (aleatoriu)":
                variant = tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "🔥 Hot Numbers (3 din top 10 + 1 din rest)":
                top10 = top_nums[:10]
                rest = top_nums[10:]
                variant = tuple(random.sample(top10, min(3, len(top10))) + random.sample(rest, max(0, numere_pe_varianta - 3))) if len(rest) > 0 else tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "❄️ Cold-Hot Hybrid (2 din top 20 + 2 din 21-50)":
                top20 = top_nums[:20]
                rest = top_nums[20:]
                variant = tuple(random.sample(top20, 2) + random.sample(rest, numere_pe_varianta - 2)) if len(rest) >= 2 else tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "🔥❄️ Premium Hybrid (3 din top 25 + 1 din rest)":
                top25 = top_nums[:25]
                rest = top_nums[25:]
                variant = tuple(random.sample(top25, 3) + random.sample(rest, numere_pe_varianta - 3)) if len(rest) > 0 else tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "⚡ Hybrid Ultra Sonic (2 din top 35 + 2 din 19–66)":
                top35 = top_nums[:35]
                rest = [n for n in top_nums if 19 <= n <= 66 and n not in top35]
                variant = tuple(random.sample(top35, 2) + random.sample(rest, numere_pe_varianta - 2)) if len(rest) >= 2 else tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "💥 Triple Fusion (3 din top 34 + 1 din rest)":
                top34 = top_nums[:34]
                rest = top_nums[34:]
                variant = tuple(random.sample(top34, 3) + random.sample(rest, numere_pe_varianta - 3)) if len(rest) > 0 else tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "🌀 Random Pairs (2 perechi aleatoare)":
                variant = tuple(random.sample(top_nums, numere_pe_varianta))

            elif strategy == "⚡ Frecvență Ponderată (pondere după frecvență)":
                weights = [st.session_state.frequency.get(n, 1) for n in top_nums]
                variant = tuple(random.choices(top_nums, weights=weights, k=numere_pe_varianta))
                ...
                if choice == "🔥":
                    top10 = top_nums[:10]
                    rest = top_nums[10:]
                    variant = tuple(random.sample(top10, 3) + random.sample(rest, numere_pe_varianta - 3))
                elif choice == "❄️":
                    top20 = top_nums[:20]
                    rest = top_nums[20:]
                    variant = tuple(random.sample(top20, 2) + random.sample(rest, numere_pe_varianta - 2))
                elif choice == "⚡":
                    top35 = top_nums[:35]
                    rest = [n for n in top_nums if 19 <= n <= 66 and n not in top35]
                    variant = tuple(random.sample(top35, 2) + random.sample(rest, numere_pe_varianta - 2))
                else:
                    top34 = top_nums[:34]
                    rest = top_nums[34:]
                    variant = tuple(random.sample(top34, 3) + random.sample(rest, numere_pe_varianta - 3))

            # 🔀 Amestecare aleatorie (nu doar sortare)
            variant = tuple(random.sample(variant, len(variant)))
            variants.add(variant)

        st.session_state.variants = sorted(list(variants))
        st.success(f"✅ Generate {len(st.session_state.variants)} variante unice ({variant_size_label})")
...
if st.session_state.variants:
    ...
    txt_output = "\n".join([f"{i+1}, {' '.join(map(str, v))}" for i, v in enumerate(st.session_state.variants, 1)])
