...
strategy = st.radio(
    ...
    [
        ...
        "⚡ Hybrid Ultra Sonic (2 din top 35 + 2 din 19–66)"  # 👈 NOUA STRATEGIE
    ]
)
        ...
        if strategy == "🔥❄️ Premium Hybrid (3 din top 25 + 1 din rest)":
            ...
            while len(variants) < num_variants and attempts < max_attempts:
                ...
                if strategy == "🎯 Standard (4 numere aleatoare)":
                    ...
                    if len(top35) >= 2 and len(rest) >= 2:
                        two_top = random.sample(top35, 2)
                        two_rest = random.sample(rest, 2)
                        variant = tuple(sorted(two_top + two_rest))
                    else:
                        variant = tuple(sorted(random.sample(top_nums, 4)))
